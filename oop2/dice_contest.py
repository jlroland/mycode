...
#    swapper_score = 0
    ...
    while game_number < number_of_games:
       # swapper.roll()
        ...
        if sum(double_roll.get_dice()) == sum(loaded_dice.get_dice()):
            #print("Draw!")
            pass
        elif sum(double_roll.get_dice()) > sum(loaded_dice.get_dice()):
            #print("Double roller wins!")
            double_roll_score+= 1
        else:
    ...
    print("Final scores")
    print("------------")
    print(f"Double roller won: {double_roll_score}")
    print(f"Loaded dice won: {loaded_dice_score}")

    # determine the winner
    if double_roll_score == loaded_dice_score:
        print("Game was drawn")
    elif double_roll_score > loaded_dice_score:
        print("Double roller won most games")
    else:
